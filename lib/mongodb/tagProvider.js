 ...
 * p: pretty name
 * d: description
 * dr: unformatted description
 * do: [] old raws for history page - Should prolly limit the bounds of this
 * u: timestamp
    ...
    if (this.current && ((Math.round((new Date()).getTime() / 1000)) - this.rebuildExpire) < this.rebuildTime) callback(null, this.list);
        ...
        this.getTagList(1,0, function(error, result) {
            for (var i = 0; i < result.length; i++) {
                ...
                if (typeof(result[i].dr) == 'undefined') {
                    result[i].dr = '';
                    c = true;
                }
                if (typeof(result[i].do) == 'undefined') {
                    result[i].do = [];
                    c = true;
                }
        ...
        this.getTags(function(error, taglist, smallList) {
            if (that.smallList.indexOf(tag.n) == -1) {
                that.getCollection(function(error, tag_collection) {
                    ...
                    tag.dr = (tag.dr) ? tag.dr : '';
                    tag.do = (tag.do) ? tag.do : [];
                    tag.c = (tag.c) ? tag.c : 0;
