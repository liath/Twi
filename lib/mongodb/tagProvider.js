        ...
        this.getTags(function(error, taglist){
            ...
            for (var i = 0; i < tags.length; i++) {
                for (var tag in that.list) {
                    if (!that.list.hasOwnProperty(tag)) continue;
                    if (tag.n == tags[i]) resp.push(tag.n);
                    else if (tag.a.indexOf(tags[i]) >= 0) resp.push(tag.n);
                }
            }
            if( error ) callback(error);
    ...
    if (tagRegex.test(tag.n)) {
        ...
        this.getTags(function() { //Make sure the below works somehow
            ...
            for (var t in that.list) {
                ...
                if (t.n === tag.n) flag=true;
                else if (t.a.indexOf(tag.n) >= 0) flag=true;
            }
            if (!flag) {
