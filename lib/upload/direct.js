...
DirectStorage.prototype.Respond = function(files, callback, hack) {
    var response = [];
            ...
            fs.rename(path, file, function(err) {
                if (err) console.log(err);
                    ...
                    imageMagick.resize({
                        ...
                        if (err) throw err
                        imageMagick.identify(file, function(err, result){
                            hack({id : name, height : result.height, width : result.width});
                        });
                    });
                }
            });
