...
var tagRegex = /^[0-9a-z_\(\)-]+$/i,
    ...
    parseMessage = function(a){
        a=a.replace(/\n/g,"<br />");
        a=a.replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig, "<a href=\"$1\">$1</a>");
        a=a.replace(/>>(\d+)/g,function(a,b){var c=$("#c"+b+" .username").html();return'<a class="commentlink" href="#c'+b+'">@'+c+"</a>"});
        return a;
    };
...
module.exports = function(app){
    ...
    app.get(/^\/wiki\/view\/([0-9a-z_\(\)-]+)(?:\/(.*))?/, function(req, res) {
        ...
        app.providers.aliasProvider.check(target, function(error, result) {
    ...
    app.post(/^\/wiki\/edit\/([0-9a-z_\(\)-]+)?(?:\/(.*))?/, function(req, res) {
        if (!req.isAuthenticated()) {
            ...
            res.redirect('/wiki/edit/'+req.params[0]);
        } else {
            var target = req.params[0];
                ...
                if (!req.body.message) {
                    ...
                    app.providers.tagProvider.getInfo(target, function(error, tagdata) {
                        ...
                        app.providers.wikiProvider.fetch(target, function(error, wiki) {
                            ...
                            if (!flag) {
                                app.providers.wikiProvider.update(wiki, function(error){
