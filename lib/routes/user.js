...
module.exports = function(app){
    smtpT = nodemailer.createTransport("SMTP", {
        ...
        app.providers.userProvider.createUser(req.body.username, req.body.password, req.body.email, function (error, user){
            if (error) {
                ...
                var email = {
                    ...
                    html : '<div>Thank you for registering '+user.u+',</div><br/><div>Click <a href="http://'+app.twi.options.domain+'/s/verify/'+user.t.k+'">here<a/> to finish!</div>'
                }
    ...
    app.get('/s/verify/:token', function(req, res){
        app.providers.userProvider.findByToken(req.param('token'), function(error, user){
            if (error) {
                ...
                app.providers.userProvider.dropToken(user, function(error, result) {
