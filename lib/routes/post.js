...
module.exports = function(app){
    ...
    app.get(/^\/post\/([a-zA-Z0-9]+)(?:\/(.*))?/, function(req, res) {
        app.providers.imageProvider.getImage(req.params[0], function(error, result) {
            if (error) {
                req.flash('error', error);
                res.redirect('/post')
            } else {
                app.providers.tagProvider.getInfo(result.t, function(error, tagdata) {
                    if (!tagdata) tagdata = [];
    ...
    app.post('/s/comment/:id', function(req, res) {
        if (!req.isAuthenticated()) {
            ...
            app.providers.imageProvider.addComment(req.param('id'), comment, function(error, result){
