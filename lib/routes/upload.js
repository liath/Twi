        ...
        if (req.isAuthenticated()) {
            ...
            if (req.session.uploads.indexOf(name) != -1) {
                ...
                for(var i = 0; i < taglist.length; i++) {
                    ...
                    var tag = { //Re-parse their slug name in the off chance someone is being sneaky I guess
                        p : taglist[i].p,
                        n : taglist[i].p.toLowerCase().replace(/[^a-z0-9-\(\)]/g, '-'),
                        m : {
                            a : req.user.u
                        }   //Grats, you just became the author of this tag.
                    };      //   assuming the tag doesn't already have an author.

                    //Inserts non-existent tags and retrieves existent ones or if the query tag was an alias it returns its parent tag.
