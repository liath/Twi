...
//Providers
var db = new Db(app.twi.options.database.name, new Server(app.twi.options.database.host, app.twi.options.database.port, {
    auto_reconnect: true
}, {}));
db.open(function() {
...
passport.use(new LocalStrategy(
    function(username, password, done) {
        app.providers.userProvider.findOne({
            ...
            if (user.t) return done(null, false, {
...
app.listen(port, function() {
    console.log("Twi booting on port %s in %s mode", port, process.env.NODE_ENV);
});